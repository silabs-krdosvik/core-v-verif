...
# print command output or not:
CMD_OUTPUT=$2

if [[ $CMD_OUTPUT == "--no-cmd-output" ]]; then
  # 'tty' represents 'current console'
  # changing 'tty' to 'null' disables output
  OUTPUT_DEV="/dev/null"
else
  OUTPUT_DEV="/dev/tty"
fi
...
usage() {
  ...
  echo "usage: $0 --[s_into_x-dv|sdev_into_xdev|xdev_into_sdev|rejection-diff] --<no-cmd-output>"
